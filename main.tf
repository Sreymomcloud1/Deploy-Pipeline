...
resource "aws_security_group" "alb_sg" {
  ...
  ingress {
    ...
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"] # Required for public access
  }
...
resource "aws_security_group" "MySG" {
  ...
  # ✅ FIX: Restrict SSH (replace with your real IP)
  ingress {
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
  ...
  # ✅ Restrict Node Exporter inside VPC only
  ingress {
    from_port   = 9100
    to_port     = 9100
    protocol    = "tcp"
...
resource "aws_launch_template" "app_lt" {
  ...
  network_interfaces {
    ...
    associate_public_ip_address = true  # Needed for ALB access
  }

  # ✅ SECURITY FIX: Enforce IMDSv2
  metadata_options {
    http_tokens = "required"
  }

  # ✅ SECURITY FIX: Encrypt root volume
